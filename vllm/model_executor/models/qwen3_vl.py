...
class Qwen3_VisionDeepstackPatchMerger(Qwen3_VisionPatchMerger):
    """Subclass for deepstack merger with distinct forward __qualname__
    to avoid AOT compile cache collision with Qwen3_VisionPatchMerger."""
...
class Qwen3_VisionTransformer(nn.Module):
    def __init__(
        ...
        with set_model_tag("Qwen3_VisionDeepstackMerger", is_encoder=True):
            self.deepstack_merger_list = nn.ModuleList(
                [
                    Qwen3_VisionDeepstackPatchMerger(
                        d_model=vision_config.out_hidden_size,
                        context_dim=self.hidden_size,
                        spatial_merge_size=self.spatial_merge_size,
